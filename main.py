...
async def edit_product(item_id: int, edit_pr: EditProducts = Body(...), current_user: UserInDB = Depends(get_current_active_user)):  # 4 +
    with db_session:
        if Products.exists(id=item_id):
...
async def delete_product(item_id: int, current_user: UserInDB = Depends(get_current_active_user)):  # 5 +
    with db_session:
        if Products.exists(id=item_id):
